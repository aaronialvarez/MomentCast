 ...
 */
async function createCloudflareStreamLiveInput(title, env) {
    try {
        const response = await fetch(`https://api.cloudflare.com/client/v4/accounts/${env.CLOUDFLARE_ACCOUNT_ID}/stream/live_inputs`, {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                meta: { name: title },
                timeoutSeconds: 0, // Never timeout - recording mode automatic
    ...
    try {
        ...
        if (pathname === '/api/events' && method === 'POST') {
            ...
            if (!body.title || !body.scheduledDate) {
                return new Response(JSON.stringify({ error: 'Missing required fields: title, scheduledDate' }), { status: 400, headers: corsHeaders });
            }
            // Check user credits
            ...
            if (!cfResult) {
                return new Response(JSON.stringify({ error: 'Failed to create live input' }), { status: 500, headers: corsHeaders });
            }
            // Generate slug
            ...
            const { data: existingEvent } = await supabase
                ...
                .single();
            if (existingEvent) {
                return new Response(JSON.stringify({ error: 'Event slug already exists' }), { status: 409, headers: corsHeaders });
            }
            // Create event
        ...
        if (pathname.match(/^\/api\/events\/[a-z0-9-]+\/status$/) && method === 'PATCH') {
            ...
            if (body.streamState)
                updates.stream_state = body.streamState;
        ...
        if (pathname.match(/^\/api\/events\/[a-z0-9-]+\/analytics$/) && method === 'GET') {
            ...
            const analyticsResponse = await fetch(`https://api.cloudflare.com/client/v4/accounts/${env.CLOUDFLARE_ACCOUNT_ID}/stream/analytics/views?creator=${event.live_input_id}`, {
                headers: {
                    'Authorization': `Bearer ${env.CLOUDFLARE_STREAM_API_TOKEN}`,
                },
            });
