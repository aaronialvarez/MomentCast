...
type View = 'sign_in' | 'sign_up' | 'check_email' | 'forgot_password' | 'reset_sent'
  ...
  if (view === 'reset_sent') {
    return (
      <Shell>
        <h1 className="text-2xl font-semibold mb-4">Check your email</h1>
        <p className="text-[var(--mc-text-2)] mb-6">
          We sent a password reset link to <strong>{email}</strong>. Click the link in your inbox to set a new password.
        </p>
        <button
          onClick={() => switchView('sign_in')}
          className="w-full px-6 py-3 border border-[var(--mc-border)] hover:bg-[var(--mc-surface-2)] rounded-lg font-medium transition-colors"
    ...
    return (
      <Shell>
        ...
        <p className="text-center text-sm mt-6">
          <button
            onClick={() => switchView('sign_in')}
            className="text-[var(--mc-text-3)] underline font-medium hover:text-[var(--mc-gold)] transition-colors"
          >
            Back to Sign In
          </button>
        </p>
        ...
        {view === 'sign_in' && (
          <div className="text-right">
            <button
              ...
              className="text-sm text-[var(--mc-text-3)] underline hover:text-[var(--mc-gold)] transition-colors"
            >
              Forgot password?
            </button>
          </div>
...
// Shared layout shell to avoid repeating the outer wrapper
